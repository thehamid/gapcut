...
export default function SeenReview({ episode, media_title, media_poster }) {
    ...
    await axios
      ...
      .then((response) => {
        ...
        if (found) {
          ...
          setSeen(false); // Optionally handle the case where media_id is not found
        }
      })
      .catch((error) => {
        console.error(
      ...
      {seen ? (
        <div className="relative flex flex-col items-center group">
          <button
            ...
            className="btn z-20 border-[1px] rounded-md text-primary  btn-sm bg-red-700  border-red-700 p-2"
          >
            <IoEyeSharp className="text-white" />
          </button>

          <div className="absolute bottom-3  flex-col items-center hidden mb-6 group-hover:flex">
            <span className="relative z-10 p-2 text-xs leading-none text-white whitespace-no-wrap bg-black shadow-lg opacity-60">
              دیده‌ام
            </span>
            <div className="w-3 h-3 -mt-2 rotate-45 bg-black opacity-60"></div>
          </div>
        </div>
      ) : (
        <div className="relative flex flex-col items-center group">
          <button
            ...
            onClick={openModal}
            data-tooltip-target="tooltip-animation"
            className="btn z-20 border-[1px] rounded-md text-primary text-white btn-sm border-red-700 p-2"
          >
            <LuEye />
          </button>
      ...
      <Transition appear show={isOpen} as={Fragment}>
        ...
        >
          <div className="min-h-screen px-4 text-center">
            ...
            >
              <div className="absolute w-full min-h-screen top-0 transit</div>ion-all transform bg-black/10  backdrop-blur-md">
